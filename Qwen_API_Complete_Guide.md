...
#### 📱 通用对话模型（推荐）
| 模型名称 | 特点 | 用途 | 定价(¥/1K tokens) |
|---------|------|------|-------------------|
| qwen-turbo | 快速、经济 | 通用对话、简单任务 | 0.0002 |
| qwen-plus | 平衡性强 | **推荐用于大多数场景** | 0.0005 |
| qwen-max | 性能最强 | 复杂推理、专业任务 | 0.002 |
| qwen-max-latest | 最新版本 | 最新功能支持 | 0.002 |
...
模型          | 输入价格    | 输出价格    | 推荐用途
-----------  | ---------- | ---------- | ----------------
qwen-turbo   | ¥0.00008   | ¥0.00016   | 简单任务、高频调用
qwen-plus    | ¥0.0005    | ¥0.0015    | 通用任务（推荐）
qwen-max     | ¥0.002     | ¥0.006     | 复杂任务、高精度
qwen-ultra   | ¥0.01      | ¥0.03      | 超高性能任务
```
...
1. **使用更便宜的模型**
   ```python
   # 优化：用qwen-turbo替代qwen-plus，成本可节省6倍
   # 但需确保精度满足要求
